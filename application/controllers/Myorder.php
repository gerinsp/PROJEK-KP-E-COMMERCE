...
class Myorder extends MY_Controller
{
    ...
    public function detail($invoice)
    {
        ...
        $data['order_detail']    = $this->myorder->select([
            ...
            'orders_detail.subtotal', 'product.title', 'product.image', 'product.price', 'orders.ongkir', 'orders.total'
        ])
            ->join('product')
            ->join('orders')
    ...
    public function posting($invoice)
    {
        ...
        if (!empty($_FILES) && $_FILES['gambar']['name'] !== '') {
            $imageName    = url_title($invoice, '-', true) . '-' . date('YmdHis');
            $upload        = $this->myorder->uploadImage('gambar', $imageName);
            if ($upload) {
                $data['input']->image    = $upload['file_name'];
            } else {
                redirect(base_url("myorder/detail/$invoice"));
        ...
        if (!$this->myorder->validate()) {
            $data['title']            = 'Konfirmasi Order';
            $data['form_action']    = base_url("myorder/detail/$invoice");
            $data['page']            = 'pages/myorder/detail';
    ...
    public function update($id)
    {
        ...
        if ($this->myorder->where('id', $id)->update(['status' => $this->input->post('status')])) {
